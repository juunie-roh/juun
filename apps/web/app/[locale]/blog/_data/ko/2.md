...
description: CesiumJS 기반 유틸리티 라이브러리를 npm 패키지로 배포하며 경험한 TypeScript, TypeDoc, Changesets, GitHub Actions, GitHub Pages를 활용한 CI/CD 구축기
created_at: 2025-04-10 09:00:00.000 +0900
updated_at: 2025-04-10 09:00:00.000 +0900
category: CASE_STUDY
image: https://raw.githubusercontent.com/npm/logos/cc343d8c50139f645d165aedfe4d375240599fd1/npm%20logo/npm-logo-red.svg
word_count: 1608
---

## Opening

사내 프로젝트에서 [CesiumJS](https://cesium.com/) 를 보다 편하게 사용하기 위해서 만들었던 여러 기능들을 개인적으로 정리해보려다가 Browser Compatibility(브라우저 호환성), Documentation, Versioning, CI/CD, npm publishing, 마지막으로 GitHub Pages Deployment 까지. 생각나는 가능한 모든 자동화를 도입해서 개발 환경을 구축한 기록을 남겨본다.

## Package Metadata

일반적인 프론트 프로젝트의 경우는 패키지를 다운 받아 사용하기만 하고 배포하지 않기 때문에 `package.json` 에 명시되는 metadata 를 신경 쓸 일이 없다. 하지만 npm 패키지 형태로 관리 및 사용하려는 경우는 패키지의 이름부터 버전, exports 등 구체적으로 명시해줘야 하는 metadata 가 많다. [Configuring npm](https://docs.npmjs.com/cli/v11/configuring-npm/package-json) 에서 모든 항목을 확인할 수 있다.
...
- name: 패키지의 이름으로, npm 에 publish 하기 위해서는 사용할 npm 계정에 publish 권한이 있는 [scope](https://docs.npmjs.com/cli/v11/using-npm/scope) 를 지정해주어야 한다. 개인 계정의 경우, username 과 같은 scope 는 기본으로 권한이 주어진다.
- version: npm 은 [semver](https://www.npmjs.com/package/semver?activeTab=readme) 모듈로 패키지의 버전을 판단하기 때문에 형식을 맞춰주어야 한다. 이는 [Semantic Versioning](https://semver.org/) 규칙을 따르는 것으로, 이 프로젝트에서는 후술할 Changesets versioning 툴을 사용했다.

## Project Configurations

### TypeScript - Builds

기본 컴파일러인 `tsc` 를 사용해도 되지만, [`tsup`](https://tsup.egoist.dev/) 이라는 번들러를 이용했다. (~~그냥~~)
...
[TypeScript JSDoc](https://www.typescriptlang.org/docs/handbook/jsdoc-supported-types.html) 에서 지원 및 미지원 태그를 확인할 수 있다. TypeScript 는 JSDoc 의 타입 표시를 생략한다. JSDoc 을 잘 작성하면 IDE 내에서 설명을 제공할 수 있을 뿐만 아니라, API 문서까지도 자동화해서 제공할 수 있다.

[TypeDoc](https://typedoc.org/index.html) 은 대표적인 Documentation Tool 중 하나이다. JSDoc 형태의 Comment 들을 정적으로 serve 할 수 있는 `html` 로 작성해준다. MarkDown(MD) 형식도 지원하기에 API 문서를 어떻게 제공할 지에 따라 선택할 수 있다. `typedoc.json` 로 Configuration 을 설정할 수 있으며 [Typedoc Themes](https://typedoc.org/documents/Themes.html) 에서는 사용할 수 있는 스타일 테마 플러그인과 커스텀 테마 작성 방법에 대해 안내하고 있다.
...
위 설정대로 local 환경에서 `typedoc` 을 실행하면 `src/index.ts` 파일이 포함하고 있는 코드의 JSDoc 에 대한 API Document 가 `docs` 디렉토리 하위에 생성된다. 그 중 `index.html` 을 [라이브 서버](https://marketplace.visualstudio.com/items/?itemName=ritwickdey.LiveServer) 로 실행해보면 [Cesium Utils](https://juunie-roh.github.io/cesium-utils/) 와 같이 API Document 가 구성되는 것을 확인해볼 수 있다.
...
- Changeset 은 [Changeset Actions](https://github.com/changesets/action) 를 통해 `CHANGELOG.md` 의 내용을 수정할 수 있는데, Changeset 의 타입 <small>(patch/minor/major)</small> 에 따라 `package.json` 의 version 을 함께 업데이트 하는 Pull Request 를 생성하도록 설정할 수 있다.
...
1. 코드의 수정과 함께 주요 변경점을 담은 changeset 을 만들어 commit 한다.
2. Changeset action 이 version update PR 을 생성한다.
3. Version Update PR 을 Repository Owner 가 수동으로 승인한다.
4. `CHANGELOG.md` 와 `package.json` 의 변화를 감지해 GitHub Release 를 생성하고, npm 에 publish 한 뒤, API Document 를 업데이트 하고 GitHub Pages 에 Deploy 한다.
...
1. `CHANGELOG.md` 에 반영된 이번 업데이트 버전에 대한 변경 사항을 포함한 GitHub Release 를 생성하고, npm publish 를 진행한다.
2. 앞선 job 이 성공하면, API Documentation 을 최신화하고 GitHub Pages 에 deploy 한다.

_**NOTE**: 이 프로젝트는 `husky` 와 `commitlint` 로 commit 메시지의 형식을 제한하고 있어 "Release version update" 라는 메시지가 겹칠 일이 없지만, 같은 환경이 아니라면 조건 설정에 유의해야 한다._
