...
**package.json exports migration**:

```diff
- ".": "./src/index.ts"              # Direct source import
+ ".": {
+   "types": "./dist/index.d.ts",    # Built type declarations
+   "import": "./dist/index.js",     # Built ESM
+   "require": "./dist/index.cjs"    # Built CJS
+ }
```
...
```yaml
- name: Build packages
  run: pnpm --filter @juun/db build && pnpm --filter @juun/api build
```

Added before Next.js type generation to ensure workspace packages have type declarations available for downstream consumers.
...
```bash
# Prevent stale OpenAPI types
pnpm api generate
git add packages/api/generated
```
