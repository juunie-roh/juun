    ...
    <Fragment>
      ...
      <Prose className="mt-8">
        ...
        <p>
          ...
          유사한 구조의 architecture 가 급부상하고 있다. Micro Frontend (MFE) 가
          그 주인공으로, 최근 이 구조를 도입하려는 시도를 심심치 않게 목격할 수
          있다. MFE 는 무엇이고, 어떤 상황에 적합할까?
        </p>

        <blockquote>
          <MessageCircleWarning /> 이 글은{" "}
          <Link href="/blog/cloud-native">
        ...
        <blockquote>
          ...
          development, where independently developed frontends are composed into
          a greater whole. - wikipedia
        </blockquote>
        <p>
          MFE 는{" "}
          <b>
          ...
          구축하여 복잡한 dependency 공유 체계를 거치지 않고 개발을 진행하는
          것을 가능하게 해준다. 이 덕분에 다른 framework 기반의 서비스를 연결할
          수 있다는 부가적인 효과도 생긴다.
        </p>
        ...
        <ul>
          <li>
            <b>Independent Deployment</b>: 서비스들이 분리되어 독립된 workflow
            를 가지게 되므로써 다른 서비스에 구애받지 않은 개발 및 배포
            프로세스를 확보할 수 있다.
          </li>
          <li>
          ...
          <li>
            <b>Team Autonomy</b>: 독립된 개발 및 배포 프로세스 구축과 기술
            스택의 분리에 따라 각 서비스를 담당하는 팀이 자율성을 갖고 서비스를
            관리할 수 있다.
          </li>
          <li>
        ...
        <p>
          ...
          해결해줄 수 있는 완벽한 기술적 대안으로 보이기 때문이다. 하지만, MFE
          의 도입은 위의 장점들을 넘어{" "}
        ...
        <ul>
          <li>
            ...
            단순한 frontend application 의 분리를 뜻하는 것이 아니다. 어떤
            방법을 택하던, MFE 는 분리된 application context 를 갖게 된다.
            따라서 global state 를 관리하기 위한 서비스는 물론, 각 서비스의
            state 들을 조율하기 위한 orchestration layer 로 작동할{" "}
          ...
          <li>
            ...
            분리되어도 통신 성능에 큰 영향이 없다. 이는 서비스 request 및
            response 가 JSON 형태로 이루어지고, 작업에 필요한 instance 들은 각
            서비스가 포함하고 있기 때문이다. MFE 의 경우는 다르다. Frontend 는
            작업이 Client Side 에서 이루어지기 때문에 response 에 브라우저에서의
            작동을 위한 JavaScript 코드 번들을 함께 보내야 한다. 또한 통신이
            http 를 통하기 때문에 컴퓨팅 자원과 상관 없이 속도의 한계가 정해져
            있다.
          </li>
          <li>
            <b>NOT for Technical Advantages</b>: MFE 의 핵심 목적은 기술적
            이점이 아닌 조직적 문제 해결에 있다. 하지만 많은 팀들이 MFE 를 성능
            향상이나 모듈화 같은 기술적 장점을 위해 도입하려 한다. 실제로는 MFE
            는{" "}
            <Link
        ...
        <p>
          ...
          측면에서 진보는 커녕, 오히려 후퇴하는 셈이다. cross-app communication
          및 coordination 을 포함하여 복잡성이 증가할 뿐더러, 성능을{" "}
          <b>
            <i>유지</i>
          </b>{" "}
        ...
        <ol>
          <li>
            ...
            (aka route proxy): Shell application 의 route 접근을 다른
            application 의 remote url 로 rewrite 하여 제공하는 방법
          </li>
          <li>
            <Link
        ...
        <h3>1. Multi-Zones</h3>
        <p>
          이 route 를 덮어쓰는 방식은 단순해보이지만 사실 MFE 구조를 제일
          정확하게 반영한 방법이다. Next.js 는 <code>next.config.js</code> 의{" "}
        ...
        <p className="mt-2">
          ...
          application 이 특정 route 패턴에 따라 요청을 해당 application 으로
          proxy 하는 구조다. 예를 들어 <code>/shop/*</code> 경로는 쇼핑
          서비스로,
  ...
  async rewrites() {
    return [
      ...
      },
      {
        ...
        <p>
          Module Federation 은 Multi-zone 의 한계를 극복하기 위해 등장한
          방법이다. 각 application 을 runtime 에서 동적으로 로드할 수 있는
          module 형태로 빌드하여, 하나의 shell application 내에서 여러 micro
          frontend 를 조합하는 방식이다.
        </p>

        <p>
          Multi-zone 과의 핵심 차이점은{" "}
          <b>Single Page Application 특성을 유지</b>
          한다는 것이다. 각 micro frontend 가 JavaScript module 로 동작하기
          때문에 page transition 시 전체 페이지를 새로 로드할 필요가 없고,
          component 단위의 세밀한 분리도 가능하다.
        </p>
  ...
  plugins: [
    new ModuleFederationPlugin({
      ...
      exposes: {
        ...
        <ul>
          <li>
            <b>Routing Orchestration</b>: 사용자의 요청을 적절한 micro frontend
            로 라우팅하고, 전체적인 네비게이션 흐름을 관리한다.
          </li>
          <li>
        ...
        <p>
          ...
          된다는 점이다. 단순한 routing 이나 module loading 을 넘어서,
          실질적으로 분산된 시스템의 coordinator 역할을 수행해야 하므로 그
          자체로 상당한 복잡성을 갖게 된다. 결국 MFE 를 도입한다는 것은 기존의
          monolithic frontend 에 추가로 이런 orchestration layer 를 구축하고
          유지보수해야 한다는 의미이기도 하다.
        </p>
        <blockquote>
          그런데 Shell Application 이 맡게 되는 책임을 살펴보면 이상한 점을
          확인할 수 있다. Frontend 에 속한 그 분류와는 전혀 맞지 않는 backend
          성격의 책임을 갖는 것이다. 많은 MFE 소개 글들이 Shell Application 을
          Frontend 서비스로 언급하지만, 과연 이를 Frontend 영역으로 분류하는
          것이 맞을까?
        </blockquote>
        ...
        <p>
          ...
          frontend 구조를 사용했지만, 2016년에 완전히 다시 작성하기로 결정했다.
          그 이유는:
        </p>
        <ul>
          <li>
          ...
          <li>
            <b>Development Friction</b>: 여러 view 에 걸친 변경사항(시각적
            스타일 업데이트 등)을 적용하기가 매우 느렸다
          </li>
        </ul>
        <p>
        ...
        <p>
          ...
          에서 발행한 글에서 Micro Frontend 의 Best Practice 로 소개된 Facebook
          의 BigPipe 서비스 개선 사례는 <b>전형적인 분류 오류</b>의 예시다.
        </p>
        <p>
          ...
          스트리밍 하는 방식이다. 이는 본질적으로 <b>백엔드 아키텍처 최적화</b>
          에 해당하며, 현재 우리가 논의하는 MFE 개념과는 완전히 다른 영역이다.
        </p>
        <p>BigPipe 의 핵심은:</p>
        <ul>
          <li>
            <b>Server-side optimization</b>: 서버에서 페이지 구성 요소들을
            병렬로 생성
          </li>
          <li>
        ...
        <p>
          ...
          Frontend 로 분류하는 것과 같은 맥락의 <b>개념적 혼동</b>이다. MFE
          영역의 미성숙함을 보여주는 또 다른 사례로, 성능 최적화 기법과 아키텍처
          패턴을 혼동하고 있는 것이다.
        </p>

        <h2>Conclusion</h2>
        <p>
          이 글에서 살펴본 바와 같이, MFE 는 아직{" "}
          <b>개념적으로 정립되지 않은</b> 미성숙한 영역이다. Shell Application
          을 Frontend 로 분류하거나, BigPipe 같은 백엔드 최적화 기법을 MFE Best
          Practice 로 소개하는 등의 사례들은 이 분야가 얼마나 혼재된 상태인지를
          보여준다.
        </p>
        <p>
          더 중요한 것은 MFE 의 본질이 <b>기술적 해결책이 아닌 조직적 문제</b>에
          있다는 점이다. MFE 도입을 고려한다면 먼저 조직적 차원에서 접근해야
          한다:
        </p>
        <ul>
          <li>
        ...
        <p>
          ...
          모듈화 같은 검증된 기법들을 먼저 고려해보는 것이 현명하다. MFE 는
          조직적 문제에 대한 <b>마지막 수단</b>이어야 하며, 그 복잡성과 비용을
          충분히 이해한 후에 신중하게 결정해야 할 아키텍처 선택이다.
        </p>
      </Prose>
    </Fragment>
  );
