    ...
    <Fragment>
      ...
      <div className="prose text-primary mt-8 max-w-none">
        ...
        <p>
          ...
          로드 속도에서 이점을 갖는다. 하지만 이 때문에{" "}
          <code>node_modules</code> 를 통해 접근하게 되는 여러 설정들을
          바꿔주어야 하는 경우가 자주 생긴다.
        </p>
...
yarnPath: [path to your yarn release]`}
        ...
        <ul>
          <li>
            ...
            <code>pnp</code> 로 지정하면 <code>.yarn/cache</code> 아래에
            모듈들이 압축된 형태(zip)로 저장된다. 이 항목을{" "}
            <code>node-modules</code> 로 지정하면 기존 npm 의 방식을 사용하게
            된다.
          </li>
          <li>
            <code>enableGlobalCache</code>: 모듈 저장 scope 를 지정한다. 이
            옵션을 지정하지 않으면 Yarn 은 모든 모듈을 글로벌 디렉토리에
            저장한다. <code>false</code> 로 지정하면 모듈을 프로젝트 루트 경로의{" "}
            <code>.yarn/cache</code> 하위에 저장하며,{" "}
            <b className="text-lg">
              Zero-install 을 사용하고 싶다면 필수로 지정
            </b>
            해주어야 하는 항목이다.
          </li>
          <li>
        ...
        <p>
          <code>yarn init -2</code> 명령을 실행하면 <code>.gitignore</code>{" "}
          파일도 자동으로 생성된다.
        </p>
        <CodeBlock
          fileName=".gitignore"
...
`}
        ...
        <p>
          ...
          <code>!.yarn/cache</code> 는 코멘트 해제, <code>.pnp.*</code> 는
          코멘트 처리)
        </p>
        <p>
          대부분 모듈의 경우는 크기가 작아 GitHub 에도 업로드가 가능하지만, 일부
          모듈의 경우 업로드 용량 제한에 걸리는 경우가 발생하기도 한다. 그럴
          때는 해당 모듈만{" "}
...
`}
        ...
        <p>
          ...
          것처럼, Dockerfile 이나 배포 환경 등에서 호환 문제를 일으킬 가능성이
          높고, reference 가 적어 참고할 만한 자료가 마땅치 않다는 점이다.
        </p>
        <p>
          특히 <b>Next.js</b> 는 빌드 과정에서 <code>node_modules</code> 를
          참조해 필요 모듈을 직접 복사하거나 node.js 환경이{" "}
          <code>.pnp.cjs</code> 파일을 참조하도록 따로 설정해주는 등, 추가적인
          작업이 필요했다. (Vercel 의 monorepo 배포 환경에서는 해결 방법을 찾지
          못했다.)
        </p>
        <p>
        ...
        <p>
          Zero-install 은 개발 환경의 모듈을 각각 설치하지 않고 공유하므로
          모듈의 버전 관리를 git 에 통합할 수 있다는 점도 이점이 될 수 있다.
