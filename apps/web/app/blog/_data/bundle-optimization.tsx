...
export default function Data() {
  return (
    <Fragment>
      ...
      <p>
        <code>html</code> 에서 기능 구현에 필요한 JavaScript 파일을 직접 불러와
        사용하는 방식은 <code>defer</code>, <code>async</code> 와 같은 옵션을
        통해 스크립트를 불러오는 방법을 조절하거나, 스크립트의 위치를 바꿔
        불러오는 시점을 조절할 수는 있지만, module 자체를 경량화하는 것과는
        거리가 멀다. 이는 module 의 불필요한 모든 부분까지 전송/다운로드 과정을
        거쳐야 하는 문제를 발생시키며, 사용자 경험의 척도가 되는{" "}
      ...
      <p>
        ...
        <Link href="https://ko.vite.dev">vite</Link> 로 대표되는 module bundler
        는 몇 가지 방법을 통해 이 문제를 해결해준다.
      </p>
...
// BUT: Barrel exports can break this!`}
      ...
      <p>
        위의 과정을 거친 JavaScript 파일들에 다음과 같은 처리를 하여 최종 번들을
        생성한다:
      </p>
      <ul>
        <li>Minify</li>
      ...
      <p>
        ...
        번들링이라고 부른다. 번들링은 많은 부분의 최적화를 진행해주지만, 확실한
        효과를 보기 위해서는 알아두어야 할 부분과 추가적으로 작업해야 할 부분이
        있다.
      </p>
...
export default analyze ? withBundleAnalyzer(nextConfig) : nextConfig;`}
  ...
  ...
      ...
      <p>
        Module Bundler 는 앞서 언급했듯 자체적으로 스크립트의 크기를 줄여주지만,
        그럼에도 스크립트 크기가 크거나 여전히 불필요한 스크립트가 남아있을 수
        있다. 이 글을 호스트하는{" "}
        <Link href="https://github.com/juunie-roh/juun">웹 프로젝트</Link>는 웹
        3D 데모(
      ...
      <p>
        그 첫 번째 방법으로는{" "}
        <Link href="https://ko.react.dev/reference/react/lazy">React.lazy</Link>{" "}
        와{" "}
      ...
      <p>
        ...
        시키는 결과를 가져온다. 특정 컴포넌트에 포함된 module 의 로드를 지연시킬
        수 있는 것이다.
      </p>
...
export type { ViewerProps };`}
      ...
      <p>
        ...
        import 해서 사용해도, 번들은 Button, CodeBlock, Popover 등을 모두 포함한
        상태로 생성된다.
      </p>
      <CodeBlock
        fileName="index.ts"
...
export {
  ...
  PopoverTrigger,
      ...
      <p>
        ...
        <Abbr title="최초 로드에 필요한 JavaScript">First Load JS</Abbr> 용량이
        2MB 를 넘어가는 상황이 발생했다. 실제로 entry point 를 거치지 않고
        컴포넌트를 개별로 import 했더니 홈페이지(<code>/</code>)의 First Load JS
        크기가 2.53MB → 853kB, 66% 감소한 것을 확인할 수 있었다.
      </p>
      ...
      <p>
        ...
        는 지원하는 프로그래밍 언어가 약 200 개를 넘어가며 First Load JS 에서 큰
        부분을 차지하고 있었다. 10 개 남짓을 사용하고 있는 본 프로젝트에서는
        그렇게 많은 언어를 모두 포함할 필요는 없다. 마침 해당 라이브러리는
        필요한 언어를 직접 등록해 사용할 수 있도록 언어가 포함되지 않은 Light
        module 과 함께, 언어를 variants 형태로 제공하고 있다.
      </p>
      <CodeBlock
...
`}
      />
      <p>
        이처럼 필요한 언어만 선택, Lightweight library variants 를 적용한 결과,{" "}
        <code>CodeBlock</code> 을 사용하는 페이지의 First Load JS 크기도 1.82MB
        → 1.1MB, 약 40% 가량 감소시킬 수 있었다. 이렇게 module 의 경량화 버전을
        제공하는 라이브러리는 그 활용에 따라 번들 크기를 크게 줄일 수 있다.
      </p>
      ...
      <Table className="mt-4">
        <TableCaption>Total Impact Summary of Bundle Optimization</TableCaption>
        <TableHeader>
          <TableRow>
      ...
      <p>
        ...
        영향을 미치지만, 그에 못지 않게 사용한 패키지가 최적화를 고려하고 있는지
        또한 매우 중요한 것을 확인할 수 있다. npm 에 패키지를 publish 하거나,
        여기에서와 같이 monorepo 구조를 통해 사용할 module 을 패키지 형태로 직접
        관리할 때 염두에 두어야 할 것이 바로 modular export 이다. Modular export
        는 패키지의 사용자들이 필요한 기능만 import 하여 사용할 수 있도록
        패키지를 기능 단위로 묶은 entry point 를 제공하는 것을 말한다.
      </p>
      <CodeBlock
        fileName="package.json"
        code={`{
  ...
  "exports": {
    ...
    "./viewer": {
      ...
      <p>
        ...
        barrel exports 사용의 주의점이나 라이브러리의 경량 버전을 사용하는 것이
        번들 크기에 큰 영향을 준다는 점은 UX 를 위한 최적화에 중요한 관점을
        제공해주었다.
      </p>
      <p>
