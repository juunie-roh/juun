...
CMD ["/bin/hello"]`}
      ...
      <p>
        ...
        여기서는 그 중에서도 final output image 의 크기를 줄이기 위한 방법을 몇
        가지 소개한다. NextJS monorepo 프로젝트를 build 한다.
      </p>
      <h3>1. Base Image</h3>
      <p>
        ...
        추천한다. Linux OS 로써 완벽하게 동작하면서도 용량은 6MB 정도 밖에 되지
        않는 경량화된 image 이다. 대부분의 공식 image 들은 alpine 태그를
      ...
      <h3>2. Multi-stage</h3>
      <p>
        적절한 Base Image 를 선택했다면, 이제는 build stage 를 나눠줄 차례이다.
        각 stage 는 일종의 세이브 포인트처럼 작동해서, <code>FROM (stage)</code>{" "}
        명령으로 다른 stage 로부터 새로운 stage 를 만들면 기존 stage 에서 작업한
        내용들을 그대로 물려받는다. 이런 특성을 활용해 모든 stage 의 바탕이 될
        stage 를 하나 만든다.
      </p>
      <h4>Base Stage</h4>
      <CodeBlock
...
RUN apk add --no-cache libc6-compat && \\
    ...
    adduser --system --uid 1001 nextjs`}
      />
      <p>
        어플리케이션이 실행될 node 가 포함된 image 를 바탕으로 Working Directory
        설정 및 환경 변수 설정, 필수 패키지 설치, 권한 설정 등 build 될 동안
        공유하게 될 기본 설정을 진행하는 stage 이다.
      </p>
      <h4>Builder Stage</h4>
      <CodeBlock
        fileName="dockerfile"
...
RUN pnpm install --frozen-lockfile && \\
    pnpm build`}
      ...
      <p>
        ...
        하는 것을 뜻한다. 물론 이렇게 지정하면 node_modules 까지 복사하기 때문에
        시간이 매우 오래 걸릴 수 밖에 없다. build 에 필요한 파일을 직접 지정해서
        복사할 수도 있고, <code>.dockerignore</code> 에서 제외할 파일들을 지정할
        수도 있다. <code>.dockerignore</code> 는 <code>.gitignore</code> 와
        비슷하게 가져가면 된다. 이 단계를 거치면 Builder Stage 의 Working
        Directory 에 build output 이 생긴다.
      </p>
      <h4>Runner Stage</h4>
      <CodeBlock
        fileName="dockerfile"
...
`}
      ...
      <p>
        ...
        stage 에 <code>serve</code> 같은 npm 패키지나 따로 작성한 Node.js 서버를
        통해 html 을 serve 하는 명령으로 끝을 맺는다. 이렇게 build 한 image 는
        모든 소스 파일을 포함한 image 와 비교했을 때 2GB 에서 273MB 까지 줄어든
        것을 확인할 수 있다.
      </p>
      <h4>standalone?</h4>
      <p>
...
CMD ["node", "apps/web/server.js"]
      ...
      <p>
        ...
        output 실행에 필요한 패키지를 구분해 필수불가결의 요소들만 포함하는 것이
        이미지 크기를 줄이는 것에 핵심적인 역할을 한다. 같은 원리로 alpine image
        의 경우 Linux 명령어 중 대표적인 <code>sudo</code> 가 포함되어 있지 않는
        등, 불필요한 요소들은 제외되어 있는 것을 확인할 수 있다.
      </p>
      ...
      <p>
        ...
        생성하는 방식과 관련이 있다. Docker 는 image 를 Immutable 로 취급하면서,
        변경 사항들을 diff layer 로 만들어 image 에 쌓는 방식으로 저장한다.
      </p>
      <CodeBlock
        fileName="dockerfile"
...
RUN git clone https://some.project.git && \\
    ...
    cd .. && rm --rf project`}
      />
      <p>
        <code>&&</code> 와 같은 inline operator 를 활용해 layer 생성을 최소화할
        수 있다. 가독성을 높이기 위해 <code>\</code> 로 줄 바꿈을 넣어줄 수도
        있다.
      </p>
      ...
      <p>
        ...
        확인하기 쉽도록 도와준다. Image Details 항목을 살펴보면, 86KB 의 공간을
        더 줄일 수 있고 어떤 파일이 중복되는지를 보여준다.
      </p>
      <p>
      ...
      <h2>Build Cache</h2>
      <p>
        앞서 살펴봤듯이 Docker image build 과정은 서버(BuildKit)에서 diff layer
        를 추가하는 방식으로 진행된다. 이 과정에서 만들어진 layer 를 build cache
        로 사용한다.{" "}
      ...
      <p>
        ...
        정보를 보면 <code>Build Cache</code> 항목이 꽤나 큰 것을 확인할 수 있다.
        마지막 build stage 만 export 하는 image 와는 달리, build cache 는 build
        과정에서 만들어지는 모든 layer 를 저장하기 때문이다. build cache 는 그
        크기 때문에 local storage 에 계속 쌓는 것은 부담스러울 수 있는데,{" "}
        <Link href="https://docs.docker.com/build/cache/backends">
          Cache Storage Backends
        </Link>{" "}
        에서는 build cache 를 사용하거나 저장할 때 접근할 수 있는 외부 저장소를
        지정하는 방법을 안내한다.
      </p>
      <p>
        <code>RUN</code> 명령어 안에 포함된 Command 가 변경되면 layer 를 새로
        만들어야 하기에, 앞서 Image size optimization 에서 <code>RUN</code>{" "}
        명령어의 호출을 최소화했던 반면, build cache 측면에서는 layer 를 나누는
        것이 더 효율적이다. 이런 특징들로 미루어보아, build stage 가 자주 변경될
        수 있는 개발 환경에서는 layer 를 많이 생성하도록 두어 build cache 를
        최대한 활용하다가 build stage 가 확정되면 image size optimization 을
        진행하는 것이 맞는 순서로 보인다.
      </p>
...
CMD ["node", "apps/web/.next/standalone/apps/web/server.js"]`}
      ...
      <p>
        ...
        local 저장소를 참고하도록 설정해 빌드 시간을 줄일 수도 있기도 하고, 아직
        적용해보지 못한 부분들이 많다. 기회가 된다면 다른 부분들에 대한 최적화도
        진행해보고 싶다.
      </p>
    </Fragment>
  );
