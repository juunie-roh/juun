...
export const metadata = {
  title: "Case Study: Publishing npm Packages with CI/CD",
  description:
...
// ...
      ...
      <ul>
        <li>
          ...
          어떻게 취급할지를 명시한다. module 이면 esm, common 이면 common js 로
          취급한다.
        </li>
        <li>main: 패키지의 기본 primary entry point.</li>
        <li>module: 패키지를 esm 방식으로 접근할 때의 primary entry point.</li>
        <li>
          files: 패키지가 포함하는 파일들로, <code>.gitignore</code> 의 반대격.
        </li>
        <li>
      ...
      <p>
        JavaScript 는 JSDoc 이라는 documentation 양식이 있다. TypeScript 에서도
        동일한 양식을 사용한다.
      </p>
      <CodeBlock
        fileName="tsdoc.ts"
...
abstract class Collection<C, I>{
  ...
      ...
      <p>
        ...
        Documentation Tool 중 하나이다. JSDoc 형태의 Comment 들을 정적으로 serve
        할 수 있는 <code>html</code> 로 작성해준다. MarkDown(MD) 형식도
  ...
  "navigationLinks": {
    ...
    "NPM": "Link to your package"
      ...
      <p>
        ...
        <code>src/index.ts</code> 파일이 포함하고 있는 코드의 JSDoc 에 대한 API
        Document 가 <code>docs</code> 디렉토리 하위에 생성된다. 그 중{" "}
      ...
      <ul>
        <li>
          npm publish 는 <code>package.json</code> 의 name 과 version 이 primary
          key 로 작용한다. 같은 패키지 이름으로 이미 존재하는 버전으로는 다시
          publish 할 수 없다.
        </li>
        <li>
          Changeset 은{" "}
          ...
          의 version 을 함께 업데이트 하는 Pull Request 를 생성하도록 설정할 수
          있다.
        </li>
        <li>
      ...
      <p>
        이런 점들을 고려해서 배포 과정을 자동화하되, 다음과 같은 배포 process 를
        설계했다.
      </p>
      ...
      <ol>
        ...
        <li>
          ...
          감지해 GitHub Release 를 생성하고, npm 에 publish 한 뒤, API Document
          를 업데이트 하고 GitHub Pages 에 Deploy 한다.
        </li>
      </ol>
      <p>
        이는 배포에 대한 의사 결정 단계를 상정하여 Repository Owner 의 PR 수락
        시점을 최종 배포 시점으로 설정한 프로세스이다. 이런 workflow 를 구현하기
        위해 Changeset action 에 포함된 npm publish 단계를 생략하고, PR 수락
        이후 npm publish 를 진행하도록 구성했다.
      </p>
...
`}
      ...
      <ol>
        ...
        <li>
          앞선 job 이 성공하면, API Documentation 을 최신화하고 GitHub Pages 에
          deploy 한다.
        </li>
      </ol>
      <p>
      ...
      <p>
        npm 패키지를 사용만 해보고 직접 올려본 것은 처음인데, 실무를 겪어보면서
        생긴 개발 업무 프로세스에 대한 개인적인 convention 을 잘 정리해서
        반영하지 않았나 싶다. Branch naming convention, branch protection rules
        등 협업을 고려한 프로세스도 기회가 되면 설계해보고 싶다.
      </p>
    </Fragment>
  );
}
